<template>
  ...
  // Simulación de los archivos disponibles. En un caso real, deberías obtener esto de una API o un estado global.
  const files = ref([
    {
      id: '1',
      firstName: 'Juan',
  ...
  // Archivo actual que se mostrará
  const file = ref({
    id: '',
    firstName: '',
    lastName: '',
  ...
  // Función para obtener el archivo correspondiente al ID
  ...
  // Cuando el componente se monta, obtener el ID de la ruta
  onMounted(() => {
    const fileId = router.currentRoute.value.params.id as string; // Obtener el ID de la ruta
    getFileById(fileId);
  });
